...
void k1_wakeup_monitor_hart(void)
{
...

    {
        ...
        uart_puts("[K1WAKE] RVBADDR target=0x"); uart_put_hex(reset_target);
        uart_puts(" C0_readback=0x"); uart_put_hex(c0);
        uart_puts(" C1_readback=0x"); uart_put_hex(c1);
        uart_puts("\n");
    }

    {
        ...
        uart_puts("[K1WAKE] PMU_CORE_STATUS=0x"); uart_put_hex(status);
...

    {
        ...
        uart_puts(" via own instance at 0x"); uart_put_hex(wakeup_addr);
        uart_puts("\n");
